...
SQStruct::SQStruct(SQSharedState *ss,isize_t nInitialSize)
{
...
void SQStruct::Remove(const SQObjectPtr &key)
{
...
void SQStruct::AllocNodes(isize_t nSize)
{
...
void SQStruct::Rehash(bool force)
{
...
SQStruct *SQStruct::Clone()
{
    SQStruct *nt=Create(_opt_ss(this),_numofnodes);
#ifdef _FAST_CLONE
...
bool SQStruct::Get(const SQObjectPtr &key,SQObjectPtr &val)
{
...
bool SQStruct::NewSlot(const SQObjectPtr &key,const SQObjectPtr &val)
{
    assert(sq_type(key) != OT_NULL);
    size_t h = HashObj(key) & (_numofnodes - 1);
    ...
    if(sq_type(mp->key) != OT_NULL) {
        ...
        size_t mph = HashObj(mp->key) & (_numofnodes - 1);
...
isize_t SQStruct::Next(bool getweakrefs,const SQObjectPtr &refpos, SQObjectPtr &outkey, SQObjectPtr &outval)
{
...
bool SQStruct::Set(const SQObjectPtr &key, const SQObjectPtr &val)
{
    _HashNode *n = _Get(key, HashObj(key) & (_numofnodes - 1));
    if (n) {
        n->val = val;
